...
This module houses the :class:`robustnessTool.attacker.Attacker` and
:class:`robustnessTool.attacker.AttackerModel` classes.

:class:`~robustnessTool.attacker.Attacker` is an internal class that should not be
imported/called from outside the library.
:class:`~robustnessTool.attacker.AttackerModel` is a "wrapper" class which is fed a
model and adds to it adversarial attack functionalities as well as other useful
options. See :meth:`robustnessTool.attacker.AttackerModel.forward` for documentation
on which arguments AttackerModel supports, and see
:meth:`robustnessTool.attacker.Attacker.forward` for the arguments pertaining to
...
:meth:`robustnessTool.attacker.Attacker.forward`, this function should never be
called directly---instead, these arguments are passed along from
:meth:`robustnessTool.attacker.AttackerModel.forward`.
"""
...
class Attacker(ch.nn.Module):
    ...
    :class:`robustnessTool.attacker.AttackerModel`, which is how models are actually
    served (AttackerModel uses this Attacker class).

    However, the :meth:`robustnessTool.Attacker.forward` function below
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
        ...
        directly; refer to :meth:`robustnessTool.attacker.AttackerModel.forward`
        for the function you should actually be calling.

        Args:
            x, target (ch.tensor) : see :meth:`robustnessTool.attacker.AttackerModel.forward`
            constraint
                ("2"|"inf"|"unconstrained"|"fourier"|:class:`~robustnessTool.attack_steps.AttackerStep`)
